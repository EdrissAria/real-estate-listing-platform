        ...
        <form action="{{ route('properties.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select id="category" name="category_id"
                    class="mt-1 p-2 block w-full border {{ $errors->has('category') ? 'border-red-500' : 'border-gray-300' }} rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <option value="" disabled selected>Select category</option>
                    @foreach($extra_data as $category)
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" id="price" name="price"
                        ...
                        placeholder="Enter property price" value="{{ old('price') }}">
                    @error('price')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" id="bedrooms" name="bedrooms"
                        ...
                        placeholder="Enter number of bedrooms" value="{{ old('bedrooms') }}">
                    @error('bedrooms')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" id="bathrooms" name="bathrooms"
                        ...
                        placeholder="Enter number of bathrooms" value="{{ old('bathrooms') }}">
                    @error('bathrooms')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" id="area" name="area"
                        ...
                        placeholder="Enter area in square feet" value="{{ old('area') }}">
                    @error('area')
                    <span class="text-sm text-red-500">{{ $message }}</span>
                    @enderror
                </div>
